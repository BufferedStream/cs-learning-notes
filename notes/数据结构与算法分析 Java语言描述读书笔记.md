...
我们首先考虑插入。设想要把 57 插入到图 4-60 的 B 树中。延树向下查找揭示出它不在树中。此时我们把它作为第 5 项添加到树叶中。注意我们可能要为此重新组织该树叶中的所有数据。然而，与磁盘访问相比（在这种情况下它还包含一次磁盘写），这项操作的开销可以忽略不记。
...
前面例子中的节点分裂之所以行得通是因为其父节点的儿子个数尚未满员。可是，如果满员了又会怎样呢？例如，
